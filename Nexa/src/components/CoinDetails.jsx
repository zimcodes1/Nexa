...
function CoinDetails({ coin, onClose }) {
    return(
        <div className="flex fixed inset-0 bg-[#0f0f0f] bg-opacity-50 flex-wrap items-start justify-center pt-5 rounded-3xl max-sm:rounded-none max-sm:h-fit">
            <div className="flex items-center justify-between w-full px-5">
                ...
                <span>
                    ...
                    <i className="fa fa-close text-gray-50 cursor-pointer p-2 text-2xl max-sm:text-lg mx-1" 
                       onClick={onClose}></i>
                </span>
            </div>
            <div className="flex items-start justify-between w-[90%] mt-2 max-sm:mt-1 max-sm:flex-col">
        ...
        <b>{coin.priceChangePercent >= 0 ? '+' : ''}{coin.priceChangeUSD}</b>
                ...
                <span className="flex flex-col max-sm:flex-row max-sm:items-center">
                    <button className="bg-green-400 text-gray-950 font-bold py-2 px-5 rounded-md hover:bg-gray-50 cursor-pointer transition duration-200 max-sm:my-2">Trade <i className="fa fa-dollar"></i></button>
                    <u className="text-gray-200 mt-1 cursor-pointer max-sm:hidden">Available Assets</u>
                </span>
               
            </div>
            <div className="flex items-start w-[90%] max-sm:w-full h-[50%] max-sm:h-fit mt-1 p-5 max-sm:p-2">
                <TradingViewWidget></TradingViewWidget>
            </div>
            <div className="flex flex-wrap w-full mx-10 max-sm:my-5 max-sm:px-5 h-[20%] max-sm:h-fit">
                <ul className="text-gray-200 w-full"><h3 className="text-lg font-bold">Token Info</h3>
                    ...
                    <li className="mt-1 flex justify-between max-sm:mt-2 max-sm:text-sm">Chain: <span><img src="/images/ton.png" alt="Ton" className="w-[20px] h-[20px] rounded-full inline"/><b className="float-end">Ton</b></span></li>
                </ul>
            </div>
        </div>
    )
